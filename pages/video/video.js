...
Page({
    ...
    data: {
        ...
        isTriggered: false, // 下拉刷新的loading控制
        imgUrl:''
    },
    ...
    *   */
    ...
    handleUpdate(event) {
        ...
        /*
            ...
            这里就反应出 find() 方法返回的结果内存指向依然是 原数组里对象 所指向的内存地址
            所有这里返回的是浅拷贝的数据
        */
        let videoItem = videoUpdateTime.find(item => item.vid === upDateObj.vid)
        if (videoItem) {
            videoItem.currentTime = event.detail.currentTime // 这里是浅拷贝数据，所以直接赋值可以更新
        } else {
    ...
    handleToLower() {
        ...
        ]; // 网易接口没有提供分页，所以要mock数据
        let { videoList } = this.data
        videoList.push(...newVideoList) // 因为要push对象，所以要解构
        this.setData({
            videoList
        })
    },
    ...
    onShareAppMessage: function ({from,target}) {
        console.log(target.dataset.imgUrl);
        if (from === 'button') {
            return {
                title: 'ggb的小程序',
                path: '/pages/video/video',
                imageUrl: target.dataset.imgurl
            }
        }else {
            return {
                title: 'ggx的小程序',
